...
class ActApkServicio: AppCompatActivity() {
    ...
    private fun comenzarActualizacion() {
        ...
        doAsync {
            if (miscServicio.descargarApk()) {
                ...
                uiThread {
                    Toast.makeText(this@ActApkServicio, "No se pudo descargar la apk", Toast.LENGTH_LONG).show()
                    finish()
                }
            }
        }
    ...
    private fun lanzarActualizacion(fVersionApk: String) {
        try {
            ...
            val localDirectory = getExternalFilesDir(null)?.path + "/actualizacion/"
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                val fileUri = FileProvider.getUriForFile(applicationContext, BuildConfig.APPLICATION_ID + ".provider", file)
